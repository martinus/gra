...
def fetch_git_config(name, run_env):
    try:
        return subprocess.check_output(
            ["git", "config", "--get", name], encoding='utf-8', env=run_env).strip()
...
class GitRepoAdmin:
    def __init__(self, args, db_file_name, root, run_env):
        self._args = args
    ...
    def _url_to_path(self, url):
        ...
        target_path = os.path.join(
            self._root_path, parsed_url.hostname) + parsed_url.path
    ...
    def _run_pre_post_command(self, name, path):
    ...
    def _rm(self):
        p = os.path.abspath(os.path.expandvars(
            os.path.expanduser(self._args.path)))
        ...
        origin_url = subprocess.check_output(
            ["git", "config", "--get", "remote.origin.url"], encoding='utf-8', cwd=p, env=self._run_env).strip()

        # show git status
        git_status = subprocess.check_output(
            ["git", "status", "--porcelain"], encoding='utf-8', cwd=p, env=self._run_env).strip()
        if git_status:
            Log.warn(
                f"WARNING: repository has modifications:\n{git_status}")
...
gra()  {
    if [ "$#" -eq 1 ]; then
        if [ "$1" = "cd" ]; then
            builtin cd -- $($__GRA ls|fzf)
            return
        fi
        if [ "$1" = "rm" ]; then
            $__GRA rm $($__GRA ls|fzf)
            return
        fi
    fi
    $__GRA $@
